    ...
    if (!breadcrumbs) {
      ...
      if (!path.length) {
        setCurrentBreadcrumb([
          ...
          {
            name: "Bookmarks",
            icon: `<svg class="h-4 w-4 shrink-0" aria-hidden="true" viewBox="0 0 256 256"> <path d="M200,32H163.7a47.8,47.8,0,0,0-71.4,0H56A16,16,0,0,0,40,48V216a16,16,0,0,0,16,16H200a16,16,0,0,0,16-16V48A16,16,0,0,0,200,32Zm-72,0a32.1,32.1,0,0,1,32,32H96A32.1,32.1,0,0,1,128,32Z"></path> </svg>`,
          },
        ])
      } else {
      ...
      <ul className="flex space-x-3 text-sm font-medium">
        {currentBreadcrumb?.map((breadcrumb, i) => (
          <li key={breadcrumb.name} className="flex items-center space-x-3">
            {i !== 0 && (
              <div aria-hidden="true" className="h-4 w-px rotate-12 rounded-full bg-gray-300" />
            )}
            <div className="flex items-center space-x-1 text-gray-800">
              {breadcrumb.icon ? (
                <svg
                  className="h-4 w-4 shrink-0 fill-slate-800"
